...
public sealed class LuxApiClient : IDisposable
{
    ...
    {
        using var resp = await _http.PostAsJsonAsync("/api/sessions", request, ct).ConfigureAwait(false);
        await EnsureSuccessAsync(resp, ct).ConfigureAwait(false);
    ...
    public async Task<string> StartTestRunAsync(string sessionId, string component, CancellationToken ct = default)
    {
        ...
        using var resp = await _http.PostAsJsonAsync($"/api/sessions/{sessionId}/test-runs", request, ct).ConfigureAwait(false);
        ...
        string sessionId, string testRunId, CompleteTestRunRequest request, CancellationToken ct = default)
    {
        using var resp = await _http.PatchAsJsonAsync(
            $"/api/sessions/{sessionId}/test-runs/{testRunId}", request, ct).ConfigureAwait(false);
        await EnsureSuccessAsync(resp, ct).ConfigureAwait(false);
    ...
    {
        using var resp = await _http.PatchAsJsonAsync($"/api/sessions/{sessionId}", new EndSessionRequest(), ct).ConfigureAwait(false);
        await EnsureSuccessAsync(resp, ct).ConfigureAwait(false);
    }
