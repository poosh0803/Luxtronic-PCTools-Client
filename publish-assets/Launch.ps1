<#
    ...
    configured server is actually reachable right now. HWiNFO is checked too but only ever as an
    informational [--]/[OK], never a [FAIL] - it's an optional CPU temp/clock fallback for some
    hardware (see README "Known risk"), not something every machine needs.
...
# reads fail - see README "Known risk") - informational only, never blocks launch.
if (Test-Path $HwiExe) {
    ...
    try {
        ...
        Write-CheckInfo 'HWiNFO shared memory not active right now - only matters if this app''s own CPU temp/clock reads come up blank. If so: launch HWiNFO64.exe, enable Settings > Shared Memory Support, then restart HWiNFO64.exe.'
    }
} else {
